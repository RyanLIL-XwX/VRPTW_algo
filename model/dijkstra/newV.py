...
class VRPTW_model(object):
    ...
    def __str__(self):
        return "VRPTW_model:(\n force_merge={},\n hand_over_time_by={},\n order_list={},\n parameters={},\n task_code={},\n vehicle_type={},\n warehouse={}\n)".format(
            self.force_merge, self.hand_over_time_by, self.order_list, self.parameters, self.task_code,
            self.vehicle_type, self.warehouse)
        # or
    ...
    def collect_location_info(self):
        ...
        # warehouse
        if (
                "address" in self.warehouse.keys() and "latitude" in self.warehouse.keys() and "longitude" in self.warehouse.keys() and
                self.warehouse["address"] != None and self.warehouse["latitude"] != None and self.warehouse[
            "longitude"] != None):
            location_collect.append(
                [self.warehouse["address"], float(self.warehouse["latitude"]), float(self.warehouse["longitude"]),
                 "仓库"])
        else:
        ...
        for i in range(len(self.order_list)):
            ...
            if ("receivingAddress" in self.order_list[i].keys() and "receivingLatitude" in self.order_list[
                i].keys() and "receivingLongitude" in self.order_list[i].keys() and self.order_list[i][
                "receivingLatitude"] != None and self.order_list[i]["receivingLongitude"] != None and
                    self.order_list[i]["receivingAddress"] != None):
                temp_list = list()
    ...
    def calculate_distance(self, location_collect):
        ...
        while True:
            ...
            distance_km = haversine((location_collect[pointer_a][1], location_collect[pointer_a][2]),
                                    (location_collect[pointer_b][1], location_collect[pointer_b][2]),
        ...
        for i in range(len(self.order_list)):
            if (self.order_list[i]["receivingAddress"] == first_order_address):
                ...
                if (self.order_list[i]["receivingAddress"] in temp_collect.keys()):
                    temp_collect[first_order_address].append(
                        (self.order_list[i]["orderCode"], self.order_list[i]["receivingEarliestTime"]))
                else:
                    temp_collect[first_order_address] = [
                        (self.order_list[i]["orderCode"], self.order_list[i]["receivingEarliestTime"])]
    ...
    def calculate_stay_period(self, item_weight):
        stay_period = round(
            item_weight * self.parameters["handoverVariableTime"] + self.parameters["handoverFixedTime"])  # 四舍五入
        return stay_period
    ...
    def get_first_order_address(self, distance_store):
        ...
        for i in (sorted_distance_store_update_copy.keys()):
            ...
            if (self.time_warehouse_leave_availble(warehouse_leave_time) == True and self.weight_availble(
                    check_weight) == True and self.volume_availble(check_volume) == True):
    ...
    def dijkstra(self, distance_store, start_address):
        ...
        while priority_queue:
            ...
            if (self.weight_availble(check_weight) == True and self.volume_availble(
                    check_volume) == True and self.time_leave_availble(self.get_receive_latest_time(current_node),
                                                                       leave_time) == True):
                pass
            else:
...
if __name__ == "__main__":
    ...
    def testbasic():
        ...
        single_receive_time = order_data.get_receive_earliest_time(
            "北京市昌平区南口镇陈庄村(京藏高速北侧)八达岭奥特莱斯F2")
        # print(single_receive_time)
        single_receive_latest_time = order_data.get_receive_latest_time(
            "北京市昌平区南口镇陈庄村(京藏高速北侧)八达岭奥特莱斯F2")
